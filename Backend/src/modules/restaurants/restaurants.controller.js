...
import { createRestaurantsSchema, updateRestaurantsSchema } from './restaurants.validation.js'
...
export const create = async (req, res, next) => {
  try {
    const { error, value } = createRestaurantsSchema.validate(req.body)
...
export const update = async (req, res, next) => {
  try {
    const { error, value } = updateRestaurantsSchema.validate(req.body)
    if (error) return sendError(res, error.details[0].message, 400)
