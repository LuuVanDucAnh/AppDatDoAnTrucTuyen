...
import { createFoodsSchema, updateFoodsSchema } from './foods.validation.js'
...
export const create = async (req, res, next) => {
  try {
    const { error, value } = createFoodsSchema.validate(req.body)
...
export const update = async (req, res, next) => {
  try {
    const { error, value } = updateFoodsSchema.validate(req.body)
    if (error) return sendError(res, error.details[0].message, 400)
